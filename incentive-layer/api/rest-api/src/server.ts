...
app.get('/api/stats', async (req: Request, res: Response) => {
    try {
      ...
      SELECT
        ...
        (SELECT COALESCE(SUM(capacity_gb), 0) FROM capacity_pledges WHERE active = true AND LOWER(provider_address) IN (SELECT LOWER(address) FROM providers WHERE last_heartbeat > NOW() - INTERVAL '45 seconds')) as total_capacity_gb,
