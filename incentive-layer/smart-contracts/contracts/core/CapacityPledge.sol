...
contract CapacityPledge is Ownable {
    ...
    event PledgeCompleted(address indexed provider, uint256 indexed pledgeId, uint256 returnedCollateral);
    event PledgeExitedEarly(address indexed provider, uint256 indexed pledgeId, uint256 penalty, uint256 returned);
    event UtilizationUpdated(address indexed provider, uint256 indexed pledgeId, uint256 newUtilization);
    ...
    function createPledge(uint256 capacityGB, uint256 duration, uint256 collateralAmount) external {
        ...
        uint256 minCollateral = calculateMinimumCollateral(capacityGB, duration);
        ...
        emit PledgeCreated(msg.sender, pledgeId, capacityGB, duration, collateralAmount);
    }
    ...
    function completePledge(uint256 pledgeId) external {
        ...
        require(block.timestamp >= pledge.startTime + pledge.duration, "Pledge period not complete");
        ...
        require(token.transfer(msg.sender, returnAmount), "Collateral return failed");
    ...
    function exitPledgeEarly(uint256 pledgeId) external {
        ...
        uint256 remaining = pledge.duration > elapsed ? pledge.duration - elapsed : 0;
        
        // Calculate penalty: (remaining / total) * 20% of collateral
        uint256 penalty = (pledge.collateral * remaining * EARLY_EXIT_PENALTY_PERCENT) / (pledge.duration * 100);
     ...
     */
    function updateUtilization(address provider, uint256 pledgeId, uint256 newUtilizationGB) external onlyOwner {
        Pledge storage pledge = pledges[provider][pledgeId];
        require(pledge.active, "Pledge not active");
        require(newUtilizationGB <= pledge.capacityGB, "Utilization exceeds capacity");
     ...
     */
    function getAvailableCapacity(address provider, uint256 pledgeId) external view returns (uint256) {
        Pledge storage pledge = pledges[provider][pledgeId];
