...
contract PaymentDistributor is Ownable, ReentrancyGuard {
    ...
    event UsageRewardPaid(address indexed provider, uint256 dealId, uint256 amount);
     ...
     */
    function distributeUsageRewards(uint256 dealId) external onlyOwner nonReentrant {
        (
            ,
            ,
            uint256 fileSizeGB,
        ...
        require(status == StorageMarketplace.DealStatus.Completed, "Deal not completed");
        ...
        uint256 perShardReward = (fileSizeGB * usageRatePerGBMonth * totalMonths) / 15;
        ...
        for (uint256 i = 0; i < providers.length; i++) {
            (,, , bool active) = marketplace.getShardAllocation(dealId, providers[i]);
    ...
    function withdrawEarnings() external nonReentrant {
        uint256 available = earnings[msg.sender].totalEarned - earnings[msg.sender].totalWithdrawn;
    ...
    function getAvailableEarnings(address provider) external view returns (uint256) {
        return earnings[provider].totalEarned - earnings[provider].totalWithdrawn;
    }
    
    /**
     * @dev Get detailed earnings breakdown
     */
    function getEarningsBreakdown(address provider) external view returns (
        uint256 capacityRewards,
        uint256 usageRewards,
        uint256 proofBonuses,
        uint256 totalEarned,
     ...
     */
    function useRepairReserve(address newProvider, uint256 amount) external onlyOwner {
     ...
     */
    function _getPledgeInfo(address provider, uint256 pledgeId) internal view returns (
        uint256 capacityGB,
        uint256 collateral,
        uint256 startTime,
        uint256 duration,
        ...
        (
            ...
            utilizationGB,
            active
        ) = pledges.getPledge(provider, pledgeId);
        
        return (capacityGB, collateral, startTime, duration, multiplier, utilizationGB, active);
    }
}
